...
private:
	...
	/*
		Если мы будем использовать здесь unordered_set, то проиграем по времени выполнения программы и по требуемой памяти.
		При использовании unordered_set, мы будем быстро заполнять данную хэш-таблицу, но впоследствии нам понадобится еще осортировать информацию об остановке,
		сложность сортировки по времени NlogN, и выделить под это новое место в памяти

		При использовании set, сложность по времени на добавление элемента будет logN, но при этом нам не надо будет ничего сортировать после и выделять новое место в памяти

		С учетом того, что во всех других местах программы нам нужна эта информация именно в отсортированном виде, я считаю, set - оптимальный выбор
	*/
