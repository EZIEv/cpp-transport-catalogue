...
void MapRenderer::Render(std::ostream& out, const set<const domain::Bus*, domain::BusPointerComparator>& buses) const {
    svg::Document doc;
    ...
    unordered_map<string_view, svg::Point> screen_coords_by_stop_name;
    for (const domain::Stop* stop : stops) {
        screen_coords_by_stop_name[stop->name] = proj(stop->coords);
    }

    // Отрисуем линии автобусных маршрутов
    ...
    for (const domain::Bus* bus : buses) {
        ...
        for (const domain::Stop* stop : bus->stops) {
            poly_line.AddPoint(screen_coords_by_stop_name[stop->name]);
        }
    ...
    // Подпишем линии автобусных маршрутов
    color_index = 0;
    for (const domain::Bus* bus : buses) {
        svg::Text underlayer;
        svg::Text text;

        underlayer.SetPosition(screen_coords_by_stop_name[bus->stops[0]->name]);
        text.SetPosition(screen_coords_by_stop_name[bus->stops[0]->name]);
        ...
        if (!(bus->is_roundtrip) && bus->stops.size() != 1 && bus->stops[0]->name != bus->stops[bus->stops.size() / 2]->name) {
            underlayer.SetPosition(screen_coords_by_stop_name[bus->stops[bus->stops.size() / 2]->name]);
            text.SetPosition(screen_coords_by_stop_name[bus->stops[bus->stops.size() / 2]->name]);
            underlayer.SetOffset(settings_.bus_label_offset);
            text.SetOffset(settings_.bus_label_offset);
            underlayer.SetFontSize(settings_.bus_label_font_size);
            text.SetFontSize(settings_.bus_label_font_size);
    ...
    // Отрисуем все остановки
    for (const domain::Stop* stop : stops) {
        svg::Circle circle;

        circle.SetCenter(screen_coords_by_stop_name[stop->name]);
    ...
    // Подпишем все остановки
    for (const domain::Stop* stop : stops) {
        svg::Text underlayer;
        svg::Text text;

        underlayer.SetPosition(screen_coords_by_stop_name[stop->name]);
        text.SetPosition(screen_coords_by_stop_name[stop->name]);
